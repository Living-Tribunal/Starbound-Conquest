...
document.addEventListener("DOMContentLoaded", function () {
  ...
  let a = (2 * Math.PI) / 6;
  let r = 40;
  let sides_color = "rgba(98, 207, 244)";
  let front_rear_color = "rgba(98, 207, 244)";
  ...
  const lightCannon = "rgba(245, 213, 112, 0.75)"; // Light yellow
  const heavyCannon = "rgba(255, 99, 132, 0.75)"; // Soft red
  const mediumCannon = "rgba(54, 162, 235, 0.75)"; // Light blue
  const plasmaCannon = "rgba(153, 102, 255, 0.75)"; // Light purple
  const railguns = "rgba(255, 159, 64, 0.75)"; // Orange
  const missiles = "rgba(75, 192, 192, 0.75)"; // Aqua green
  const particleBeam = "rgba(201, 203, 207, 0.75)"; // Light gray
  ...
  Object.assign(window, {
    ...
    add_planet_moon: () => add_planet_moon(shipImages, socket),
  ...
  window.ship_toggle = function () {
    if (selectedShip) {
      console.log("Toggled")
      selectedShip.isToggled = true;
      socket.emit("shipToggleUpdate", { id: selectedShip.id, hp: selectedShip.isToggled });
      save_canvas();
      draw_scene();
    } else {
      alert("No ship was selected to update");
...
window.reset_ship_status = function(){
  ships.forEach((ship) => {
    ship.isToggled = false;
  })
  save_canvas();
  draw_scene();
  alert("All toggles have been reset");
  };
  ...
  socket.on("shipToggleUpdate", function (shipData) {
    let index = ships.findIndex((s) => s.id === shipData.id);
    if (index !== -1) {
      ships[index].hp = shipData.isToggled;
      draw_scene();
    }
  });
...
window.load_canvas = function(){
  document.getElementById("loadButton").disabled = true;
  document.getElementById("acknowledge").style.opacity = 1;
  document.getElementById("load-close").disabled = false;
  document.getElementById("load-close").style.opacity = 1;
        ...
        shipsData.forEach((shipData) => {
          ...
          ship_image.onload = function () {
            let ship = {
              ...
              highlighted: false,
              isSelected: false,
              isToggled: shipData.isToggled === 1,
            };
            ships.push(ship);
            shipImages[shipData.id] = ship_image;
            draw_scene();
  ...
  function toggle_status(ship){
    let y = -ship.height / 2;
    let x = 50;
    if (ship.isToggled === true) {
      context.beginPath();
  ...
  function draw_ship(ship) {
    /* console.log(`Drawing ship with ID: ${ship.id}`);
    console.log("Total ships loaded:", ships.length);
    ...
    if (!is_dragging && !is_panning) {
      switch (ship.type) {
        case "Fighter":
          ...
          context.lineWidth = 5;
          context.arc(0, 0, light, 1.25 * Math.PI, -0.75);
        ...
        case "Destroyer":
          ...
          context.fillStyle = "transparent";
          context.lineWidth = 5;
          context.arc(0, 0, mediumBlaster, 1.25 * Math.PI, -0.75);
          context.lineCap = "round";
          context.stroke();
          context.closePath();
          context.fill();
        ...
        case "Cruiser":
          ...
          context.fillStyle = "transparent";
          context.lineWidth = 5;
          context.arc(0, 0, heavy, 1.25 * Math.PI, -0.75);
          context.lineCap = "round";
          context.stroke();
          context.closePath();
          context.fill();
          ...
          //plasma cannon - right
          context.beginPath();
          context.strokeStyle = plasmaCannon; //right firing angle
          context.fillStyle = "transparent";
          context.lineWidth = 5;
          context.arc(0, 0, dreadPlasma, -0.75, 0.25 * Math.PI);
          context.lineCap = "round";
          context.stroke();
          context.closePath();
          context.fill();
          ...
          //plasma cannon - left
          context.beginPath();
          context.strokeStyle = plasmaCannon; //left firing angle
          context.fillStyle = "transparent";
          context.lineWidth = 5;
          context.arc(0, 0, dreadPlasma, 0.75 * Math.PI, 1.25 * Math.PI);
          context.lineCap = "round";
          context.stroke();
          context.closePath();
          context.fill();
          break;
        case "Carrier":
          //350 railgun front - outer
          context.beginPath();
          context.strokeStyle = railguns;
          context.fillStyle = "transparent";
          context.lineWidth = 5;
          context.arc(0, 0, rail, 1.25 * Math.PI, -0.75);
          context.lineCap = "round";
          context.stroke();
          context.closePath();
          context.fill();

          //350 railgun front - inner
          context.beginPath();
          context.strokeStyle = railguns;
          context.fillStyle = "transparent";
          context.lineWidth = 5;
          ...
          //missiel launches - inner
          context.beginPath();
          context.strokeStyle = missiles;
          context.lineWidth = 5;
          context.fillStyle = "transparent";
          context.arc(0, 0, light / 2, 0, 2 * Math.PI);
          context.lineCap = "round";
          context.stroke();
          context.closePath();
          context.fill();
        ...
        case "Dreadnought":
          ...
          context.fillStyle = "transparent";
          context.lineWidth = 5;
          context.arc(0, 0, particle, 1.25 * Math.PI, -0.75);
          context.lineCap = "round";
          context.stroke();
          context.closePath();
          context.fill();
  ...
  function draw_zoom_percentage() {
    ...
    if (zoom < 1) {
      context.beginPath();
      context.rect(334, 15, 170, 50);
      context.fill();
      context.stroke();
      context.fillStyle = "white";
      context.fillText(`Zoom: ${(zoom * 100).toFixed(0)}%`, 350, 50);
      context.restore();
    }
  }
  ...
  function draw_selected_ship_info() {
    if (selectedShip) {
      ...
      if (selectedShip.type != "CelestialBodies") {
        ...
        context.fillText(`Ship ID: ${selectedShip.shipId}`, 10, 100);
  ...
  function traveled_distance(ship){
    let firstY = -ship.height / 2;
    let firstX = 150;

    if (ship.isSelected) {
      let deltaX = ship.x - ship.prevX;
      let deltaY = ship.y - ship.prevY;
      let distance_traveled = Math.sqrt(deltaX * deltaX + deltaY * deltaY) / 14;
      
      context.font = "25px monospace";
      context.fillStyle = "gold";
      context.fillText(`>: ${distance_traveled.toFixed(0)}ft`, firstX, firstY + 6);
    } else {
      ship.prevX = ship.x;
      ship.prevY = ship.y;
    }
