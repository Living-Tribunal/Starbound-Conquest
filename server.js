...
io.on('connection', (socket) => {
    ...
    socket.on('updateShipToggle', (shipData) => {
      let index = ships.findIndex(s => s.id === shipData.id);
      if (index !== -1) {
          ships[index].isToggled = shipData.isToggled;
          io.emit('shipToggleUpdate', ships[index]);
      }
  });
        ...
        db.run('DELETE FROM ships WHERE id NOT IN (' + currentShipIds.map(() => '?').join(', ') + ')', currentShipIds, (err) => {
            ...
            const statement = db.prepare(`
                INSERT OR REPLACE INTO ships (
                    id, shipId, type, x, y, prevX, prevY, width, height, isSelected, isToggled, rotation_angle, highlighted, image, globalAlpha, maxHP, hp, damageThreshold, threatLevel, moveDistance, weaponType, firingArc, WeaponDamage, weaponRange, pointvalue
                ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            `);
